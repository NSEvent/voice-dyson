...
def send_swivel():
    """Use LIRC to send swivel signal."""
    subprocess.run(['irsend','SEND_ONCE',REMOTE_CONF,'KEY_WAKEUP'])
    time.sleep(0.25)
    # # TODO:
    return
...
def send_temp_mode(temp_mode):
    ...
    assert(temp_mode == 'hot' or temp_mode == 'cold')

    temp_button = None
    if temp_mode == 'hot':
        temp_button = 'KEY_RED'
    elif temp_mode == 'cold':
        temp_button = 'KEY_BLUE'
    subprocess.run(['irsend','SEND_ONCE',REMOTE_CONF,temp_button])
    time.sleep(0.25)
    # # TODO:
...
def send_power_level(power_level):
    ...
    for _ in range(10):
        subprocess.run(['irsend','SEND_ONCE',REMOTE_CONF,'KEY_DOWN'])
        time.sleep(0.25)

    for _ in range(int(power_level) - 1):
        subprocess.run(['irsend','SEND_ONCE',REMOTE_CONF,'KEY_UP'])
        time.sleep(0.25)
...
# def set_fan_settings(temp_mode, power_level, temp_level, power_mode):
    ...
    if temp_level and temp_mode == 'hot':
